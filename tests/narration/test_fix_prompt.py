...
def test_module_contract():
    import querysmith.narration.fix_prompt as module

    assert module.__all__ == ["build_fix_prompt"]


def test_prompt_contains_plan_summary_fields(op_factory, plan_factory):
    plan = plan_factory(op_factory(), statement_text="SELECT * FROM dbo.Orders")
    summary = summarize_plan(plan)
    prompt = build_fix_prompt(summary, [])
...
def test_prompt_contains_finding_index_and_fields(finding_factory):
    ...
    prompt = build_fix_prompt(_dummy_summary(), findings)
...
def test_prompt_contains_schema_instructions():
    prompt = build_fix_prompt(_dummy_summary(), [])
    assert "finding_index" in prompt
    assert "rewritten_query" in prompt
    assert "index_script" in prompt
...
def test_prompt_is_deterministic(finding_factory):
    ...
    assert build_fix_prompt(summary, findings) == build_fix_prompt(summary, findings)
